...
export default function useCalculateMathSeries(): {
  ...
  addDisabledMathSeriesFormEarseValues: (matSeriesForm: MathSeriesFormModel) => void;
  ...
  const [processedMathSeriesCalculationPetition, setProcessedMathSeriesCalculationPetition] = useState<boolean>(false)

  const [mathSeriesResultValuesList, setMathSeriesResultValuesList]: [number[][], Dispatch<SetStateAction<number[][]>>] = useState<number[][]>([])

  const [mathSeriesFormList, setMathSeriesFormList]: [MathSeriesFormModel[], Dispatch<SetStateAction<MathSeriesFormModel[]>>] = useState<MathSeriesFormModel[]>([])

  const {mathFunctionsResultValues, convertMathFunctionForm } = useCalculateMathFunction()

  const [successionFormIndexToAdd, setSuccessionFormIndexToAdd] = useState<number>(0)

  const [calculatedMathFunction, setCalculatedMathFunction] = useState<boolean>(false)
  ...
  function calculateSeriesAddOrResetForms(form: MathSeriesFormModel): void {
    ...
    if (!!form  && !isSomeImportantAtributeMathSeriesNull(form)) {
      if (tmpMathSeriesFormList.length > 0) {
        if (!!tmpMathSeriesFormList.find((mathSeriesForm) => mathSeriesForm.seriesVarName === form.seriesVarName)) {
          tmpMathSeriesFormList = tmpMathSeriesFormList.map((mathSeriesForm) => {
      ...
      if (form.isArithmeticProgression) {
        ...
        tmpParamList[Constants.differenceParameterArithmeticProgressionFormula] = `${form.difference}`

        tmpParamList[Constants.initialValueArithmeticProgressionFormula] = `${form.initialNValue}`

        tmpVarList[Constants.variableArithmeticProgressionFormula] = getRangeFromTo(
        ...
        tmpVarList[Constants.variableArithmeticProgressionFormula] = getRangeFromTo(form.initialNValue, form.finalNValue)
      }
  ...
  function addDisabledMathSeriesFormEarseValues(matSeriesForm: MathSeriesFormModel): void {
    if (!!mathSeriesFormList && mathSeriesFormList.length > 0 && !!mathSeriesResultValuesList) {
      ...
      const mathSeriesDisableIndex = mathSeriesFormList.findIndex((mathSeries) => mathSeries.seriesVarName === matSeriesForm.seriesVarName)

      if (mathSeriesDisableIndex > -1) {
        tmpMathSeriesFormList[mathSeriesDisableIndex] = matSeriesForm
      }

      setMathSeriesFormList(!!tmpMathSeriesFormList ? tmpMathSeriesFormList : [])
      setMathSeriesResultValuesList(!!tmpMathSeriesResultValuesList ? tmpMathSeriesResultValuesList : [])
      setProcessedMathSeriesCalculationPetition(!processedMathSeriesCalculationPetition)
    }
  }
    ...
    if (!!mathSeriesFormList && mathSeriesFormList.length > 0 && !!mathSeriesResultValuesList) {
      ...
      mathSeriesResultValuesList.forEach((valueList, index) => {
        if (index !== parseInt(seriesVarname.substring(3))) {
          tmpMathSeriesResultValuesList[index > parseInt(seriesVarname.substring(3)) ? index - 1 : index] = valueList
        }
      })

      mathSeriesFormList.forEach((matseries) => {
        if (parseInt(matseries.seriesVarName.substring(3)) !== parseInt(seriesVarname.substring(3))) {
          if (parseInt(matseries.seriesVarName.substring(3)) > parseInt(seriesVarname.substring(3))) {
            matseries.seriesVarName = `${Constants.VAR}${parseInt(matseries.seriesVarName.substring(3)) - 1}`
            tmpMathSeriesFormList[parseInt(matseries.seriesVarName.substring(3)) - 1] = matseries
          } else {
            tmpMathSeriesFormList[parseInt(matseries.seriesVarName.substring(3))] = matseries
          }
        }
      })
  ...
  useEffect(() => {
    if (!!mathSeriesFormList && mathSeriesFormList.length > 0) {
      const tmpMathSeriesResultValuesList = !!mathSeriesResultValuesList ? mathSeriesResultValuesList : []
      ...
      if (
        ...
        tmpSiftedValues.unshift(mathSeriesFormList[successionFormIndexToAdd].initialNValue)
        tmpMathSeriesResultValuesList[successionFormIndexToAdd] = tmpSiftedValues
        setMathSeriesResultValuesList(tmpMathSeriesResultValuesList)
      } else {
        tmpMathSeriesResultValuesList[successionFormIndexToAdd] = mathFunctionsResultValues[0].slice()
        setMathSeriesResultValuesList(tmpMathSeriesResultValuesList)
      }
      setProcessedMathSeriesCalculationPetition(!processedMathSeriesCalculationPetition)
    }
