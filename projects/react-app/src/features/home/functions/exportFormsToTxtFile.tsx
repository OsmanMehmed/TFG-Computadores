...
export default function exportFormsToTxtFile(
  exportableForm: ExportableImportableFormModel,
): void {
  const tmpExportableForm = { ...exportableForm };
  let formattedData = "";

  tmpExportableForm.mathFunctionForm.functions.forEach(
    (functionExpression, index) => {
  ...
  const numberOfRows = variableValues.reduce(
  ...
  formattedData += headerRow
    .map((h) =>
      h.padEnd(
        longestValueLength(variableValues, paramValues) > minimumPaddingEnd
          ? longestValueLength(variableValues, paramValues) + 1
          : minimumPaddingEnd,
      ),
    )
    .join("");
  ...
  Object.entries(tmpExportableForm.mathFunctionForm.varList).forEach(
    ([varName, varValues]) => {
  ...
  Object.entries(tmpExportableForm.mathFunctionForm.paramList).forEach(
  ...
  if (!!tmpExportableForm.mathSeriesFormList) {
    const tmpMathSeriesFormModelList = Object.values(
      tmpExportableForm.mathSeriesFormList,
    ).map((form) => {
