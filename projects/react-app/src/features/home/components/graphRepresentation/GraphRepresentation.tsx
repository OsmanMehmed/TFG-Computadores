...
const GrapRepresentation = React.memo((props: Props): JSX.Element => {
  ...
  const [dataDependingOfNumberOfFunctions, setDataDependingOfNumberOfFunctions] = useState<Data[]>([])

  const [layoutDependingOfNumberOfFunctions, setLayoutDependingOfNumberOfFunctions] = useState<Partial<Layout>>(undefined)

  const [configDependingOfNumberOfFunctions, setConfigDependingOfNumberOfFunctions] = useState<Partial<Config>>(undefined)
  ...
  function loadPlotDataLayoutConfig(): void {
    ...
    if (!!props.independentVariables) {
      varNames = Object?.entries(props.independentVariables).map(([varName, varValues]) => `${varName}`)       
      varValues = Object.entries(props.independentVariables).map(([varName, varValues]) => varValues.map((stringNumber) => parseFloat(stringNumber)))
    } 
    

    setDataDependingOfNumberOfFunctions(props.dependantVariable.values.map( (dependantFunctionResults, index): Data  => {
      if (varValues.length === 2) {
        return {
          ...
          y: varValues[1],
          marker: { color: (index === 0 ? Constants.primaryPaletteColor : getRandomColorHexadecimal())},
        }
      } else {
        return {
          x: varValues[0],
  ...
  return (
    <Fragment>
      {!props.adjustingSize && (!!props.independentVariables ? (Object.entries(props.independentVariables).length < 3) : true) ? ( 
        (!!props.independentVariables ? (Object.entries(props.independentVariables).length === 2) : false) ? (
          <Plot 
            className="plot"
        ...
        <Fragment>
          <div className='message__container'>
            <span className='message__text'>{ props.adjustingSize ? Constants.AdjustingSize : Constants.excededDimensions}</span>
          </div>
        </Fragment>
      )}
    </Fragment>
