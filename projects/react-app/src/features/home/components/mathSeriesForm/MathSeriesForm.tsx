...
const MathSeriesForm = React.memo(
  (props: MathSeriesFormPropsModel): React.ReactElement => {
    const handleFormChange = (form: MathSeriesFormModel): void => {
      props.onFormChange({ ...form });
    };

    const [successionExpressionPlaceholder] = useState<string>(
      Constants.MathSeriesPlaceHolderList[
        Math.floor(Math.random() * Constants.MathSeriesPlaceHolderList.length)
      ],
    );

    const [
      isMaxElementsSuccessionSnackBarActive,
      setIsMaxElementsSuccessionSnackBarActive,
    ...
    function handleInputChange(
      value: string,
      fieldToUpdate: MathSeriesFormModelKey,
    ): void {
    ...
    function handleInputNChange(
      ...
      if (!event.target.value) {
        ...
        if (props.isModeProgression) {
          ...
          setError("lastPossibleNValue", {
            type: "onlyIntegers",
            message: `${Constants.OnlyIntegers}`,
          });
        }
        ...
        !props.isModeProgression
          ? parseFloat(event.target.value) - initialNValue > 1000
          : parseFloat(event.target.value) > 1000
      ) {
        setIsMaxElementsSuccessionSnackBarActive(true);
        setError("lastPossibleNValue", {
          type: "maxElementLength",
          message: `${Constants.MaxElementLength}`,
        });
        handleInputChange(undefined, "finalNValue");
      } else {
    ...
    function handleInputinitialNChange(
      event: React.ChangeEvent<HTMLInputElement>,
    ): void {
      if (!event.target.value) {
        setError("initialNValue", {
          type: "required",
          message: `${Constants.Required}`,
        });
        ...
        setError("initialNValue", {
          type: "mustBeLessThanFinal",
          message: `${Constants.LessThanInitial}`,
        });
        handleInputChange(undefined, "initialNValue");
      } else if (
        !props.isModeProgression
          ? lastPossibleNValue - parseFloat(event.target.value) > 1000
          : false
      ) {
        setIsMaxElementsSuccessionSnackBarActive(true);
        setError("initialNValue", {
    ...
    function handleInputDifferenceBetweenNValuesChange(
      event: React.ChangeEvent<HTMLInputElement>,
    ): void {
      if (!event.target.value) {
        setError("differenceBetweenNValues", {
          type: "required",
          message: `${Constants.Required}`,
        });
    ...
    function handleInputFunctionChange(
      event: React.ChangeEvent<HTMLInputElement>,
    ): void {
      if (!event.target.value) {
        setError("seriesFunction", {
          type: "required",
          message: `${Constants.Required}`,
        });
        ...
        Object.entries(
          getFunctionParameters(
            ...
            [],
            {},
            Constants.SuccessionFunctionReservedWords,
          ),
        ).length > 0
      ) {
        setError("seriesFunction", {
          type: "onlyReservedWords",
          message: `${Constants.SuccessionFormulaErrorOnlyReservedWords}`,
        });
        handleInputChange(undefined, "seriesFunction");
      } else if (
        event.target.value.includes("N") ||
        !/^(?!.*n\s+n)(?!.*n{2,}).*$/.test(event.target.value)
      ) {
        setError("seriesFunction", {
          type: "badExpression",
    ...
    return (
      <Fragment>
        <Stack direction="row" className="form-series">
          <FormControl
            error={errors.seriesFunction ? true : false}
            className="input-series-function"
          >
                  ...
                  <InputAdornment position="start">
                    <div className="function-start input-adornments">
                      ...
                      {(!!initialNValue || initialNValue >= 0) && (
                        <span>{`${Constants.BlankSpace}${cutIfLongName(`${initialNValue}`, Constants.cutParamNameIfGreatherThan)}`}</span>
                      )}
                      {!initialNValue && initialNValue !== 0 && (
                        <span>
          ...
          <Stack direction="column" spacing={1}>
            <FormControl
              error={errors.lastPossibleNValue ? true : false}
              className="succesion-config-control"
            >
              {!!errors.lastPossibleNValue?.message ? (
                ...
                <InputLabel>
                  {props.isModeProgression
                    ? `${Constants.NBS.repeat(4)}${Constants.NumberElements}`
                    : `${Constants.NBS.repeat(4)}${Constants.LastNValue}`}
                </InputLabel>
              )}
              <OutlinedInput
                label={
                  errors.lastPossibleNValue?.message
                    ? `${errors.lastPossibleNValue?.message}`
                    : props.isModeProgression
                      ? `${Constants.NumberElements}`
                      : `${Constants.LastNValue}`
                }
            ...
            <FormControl
              error={errors.initialNValue ? true : false}
              className="succesion-config-control"
            >
              {errors.initialNValue?.message ? (
                ...
                <InputLabel>
                  {props.isModeProgression
                    ? `${Constants.NBS.repeat(4)}${Constants.FirstValue}`
                    : `${Constants.NBS.repeat(4)}${Constants.FirstnValue}`}
                </InputLabel>
              )}
              <OutlinedInput
                label={
            ...
            {!!props.isModeProgression && (
              <FormControl
                error={errors.differenceBetweenNValues ? true : false}
                className="succesion-config-control"
              >
                {errors.differenceBetweenNValues?.message ? (
                  ...
                  <InputLabel>
                    {props.isModeProgression
                      ? `${Constants.NBS.repeat(4)}${Constants.ValuesDifference}`
                      : ""}
                  </InputLabel>
                )}
                <OutlinedInput
